...
static void background_done(Background* back) {
    skin_image_unref(&back->image);
}

static void background_init(Background* back,
...
static void background_redraw(Background* back,
                              SkinRect* rect,
                              SkinSurface* surface) {
    SkinRect r;
    ...
    if (skin_rect_intersect(&r, rect, &back->rect)) {
        ...
        getConsoleAgents()->surface->skin_surface_blit(
                surface, &r.pos, skin_image_surface(back->image), &src_rect,
                SKIN_BLIT_SRCOVER);
    }
}
...
static void adisplay_done(ADisplay* disp) {
    ...
    getConsoleAgents()->surface->skin_surface_unrefp(&disp->surface);
    disp->data = NULL;
...
static int adisplay_init(ADisplay* disp,
    ...
    if (sdisp->framebuffer_funcs) {
        disp->data = sdisp->framebuffer_funcs->get_pixels(sdisp->framebuffer);
        disp->bits_per_pixel =
...
static void dotmatrix_dither_argb32(unsigned char* pixels,
    ...
    static const unsigned dotmatrix_argb32[16] = {
            0x003f00, 0x00003f, 0x3f0000, 0x000000, 0x3f3f3f, 0x000000,
            0x3f3f3f, 0x000000, 0x3f0000, 0x000000, 0x003f00, 0x00003f,
...
static struct local_pixels_buffer_t local_pixels_buffer = {.pixels = NULL,
                                                           .size = 0};
...
static void adisplay_update_surface(ADisplay* disp, SkinRect* rect) {
    int x = rect->pos.x;
    int y = rect->pos.y;
    int w = rect->size.w;
    int h = rect->size.h;
    ...
    // content.
    int sz = disp->datasize.h > disp->datasize.w ? disp->datasize.h
                                                 : disp->datasize.w;
    int dst_pitch = 4 * sz;
    ...
    SkinRect dst_r = {.pos.x = x, .pos.y = y, .size.w = w, .size.h = h};
    ...
    if (disp->gpu_frame) {
        ...
        if (disp->bits_per_pixel == 32) {
            adisplay_update_surface_pixels_32(disp, &dst_r, dst_pixels,
                                              dst_pitch, disp->data);
        } else {
            adisplay_update_surface_pixels_16(disp, &dst_r, dst_pixels,
                                              dst_pitch);
        }
    }

    // Apply brightness modulation.
    lcd_brightness_argb32((uint32_t*)dst_pixels, disp->datasize.w,
                          disp->datasize.h, dst_pitch, disp->brightness);

    // Update the display surface content
    getConsoleAgents()->surface->skin_surface_upload(disp->surface, &dst_r, dst_pixels, dst_pitch);
...
static void adisplay_redraw(ADisplay* disp,
    ...
    if (disp->brightness == LCD_BRIGHTNESS_OFF) {
        ...
        getConsoleAgents()->surface->skin_surface_fill(surface, &r, 0xff000000);
    } else {
        getConsoleAgents()->surface->skin_surface_blit(surface, &r.pos, disp->surface, &src_r,
                          SKIN_BLIT_COPY);
    }
    ...
    if (disp->onion != NULL) {
        ...
        if (skin_rect_intersect(&r2, &r, &disp->onion_rect)) {
            ...
            getConsoleAgents()->surface->skin_surface_blit(surface, &r2.pos, skin_image_surface(disp->onion),
                              &src_rect, SKIN_BLIT_SRCOVER);
        }
    }

    getConsoleAgents()->surface->skin_surface_update(surface, &r);
}
...
static void button_done(Button* button) {
    skin_image_unref(&button->image);
    button->background = NULL;
}

static void button_init(Button* button,
                        SkinButton* sbutton,
                        SkinLocation* loc,
                        Background* back,
                        SkinRect* frame,
    ...
    if (slayout->has_dpad_rotation) {
        /* Dpad keys must be rotated if the skin provides a 'dpad-rotation'
         * field. this is used as a counter-measure to the fact that the
         * framework always assumes that the physical D-Pad has been rotated
         * when in landscape mode.
         */
        button->keycode =
                skin_keycode_rotate(button->keycode, -slayout->dpad_rotation);
    }
...
static void finger_state_reset(FingerState* finger) {
    finger->tracking = 0;
    finger->inside = 0;
    finger->at_corner = 0;
    finger->pos.x = 0;
...
static void button_state_reset(ButtonState* button) {
    button->pressed = NULL;
    button->hover = NULL;
}
...
static void ball_state_reset(BallState* state, SkinWindow* window) {
    state->tracking = 0;
    state->ball = NULL;
...
static void ball_state_redraw(BallState* state,
                              SkinRect* rect,
                              SkinSurface* surface) {
    SkinRect r;
...
static void ball_state_show(BallState* state, int enable) {
    if (enable) {
        if (!state->tracking) {
            state->tracking = 1;
...
static void ball_state_set(BallState* state, SkinTrackBall* ball) {
...
static void layout_done(Layout* layout) {
    int nn;
...
static int layout_init(Layout* layout, SkinLayout* slayout) {
    ...
    if (layout->buttons == NULL && n_buttons > 0)
        goto Fail;
    if (layout->backgrounds == NULL && n_backgrounds > 0)
        goto Fail;
    if (layout->displays == NULL && n_displays > 0)
        goto Fail;
...
static void generate_touch_event(FingerState* finger,
                                 int* x,
                                 int* y,
                                 int* device_id,
                                 uint32_t display_id) {
    ...
    if (finger->display) {
        if (android_foldable_is_folded() && not_pixel_fold) {
            ...
            if (!multiDisplayAgent->isMultiDisplayWindow()) {
                multiDisplayAgent->translateCoordination(&posX, &posY,
                                                         &display_id);
            }
        }
    }
...
static void skin_window_find_finger(SkinWindow* window,
                                    FingerState* finger,
                                    int x,
                                    int y,
                                    int displayId) {
    ...
    if (multi_display) {
        ...
        if (finger->inside) {
            ...
            skin_pos_rotate(&finger->pos, &finger->pos,
                            multi_display->rotation);
            skin_winsys_set_window_cursor_normal();
        }
        return;
    }

    bool roundDevice = getConsoleAgents()->surface->skin_surface_is_round(window->surface);
    ...
    if (roundDevice) {
        ...
        if (y >= disp->rect.pos.y + disp->rect.size.h) {
            ...
            finger->inside =
                    distSquared <= (disp->rect.size.w * disp->rect.size.w) / 4;
        }
    } else {
        // Rectangular device
    ...
    if ((x < leftEdge || x >= rightEdge) && (y < topEdge || y >= bottomEdge) &&
        ...
        if (x < leftEdge) {
            finger->which_corner =
                    (y < topEdge) ? CORNER_TOP_LEFT : CORNER_BOTTOM_LEFT;
        } else {
            finger->which_corner =
                    (y < topEdge) ? CORNER_TOP_RIGHT : CORNER_BOTTOM_RIGHT;
        }
        skin_winsys_set_window_cursor_resize(
                finger->which_corner == CORNER_BOTTOM_LEFT ||
...
static void skin_window_move_mouse(SkinWindow* window,
                                   FingerState* finger,
                                   int x,
                                   int y) {
...
static void skin_window_trackball_press(SkinWindow* window, int down) {
    window->win_funcs->key_event(LINUX_BTN_MOUSE, down);
}

static void skin_window_trackball_move(SkinWindow* window, int xrel, int yrel) {
...
void skin_window_set_trackball(SkinWindow* window, SkinTrackBall* ball) {
    BallState* state = &window->ball;

    ball_state_set(state, ball);
}

void skin_window_show_trackball(SkinWindow* window, int enable) {
    BallState* state = &window->ball;
...
static void skin_window_setup_opengles_subwindow(SkinWindow* window,
                                                 gles_show_data* data) {
    data->window = window;
    ...
    // The native GL subwindow for OSX (using Cocoa) uses cartesian (y-up)
    // coordinates. We have code to transform window (y-down) coordinates into
    // cartesian coordinates at that level, but Qt seems to do this
    // transformation on its own. This means the transformation is done *twice*
    // with Qt + OSX, resulting in the incorrect y value. The following "undoes"
    // the transformation by doing it a third time. Additionally, because the
    // scroll bar now affects the relative coordinate system inside the window,
    // it must be taken into account as well. At the end of this function,
    // data->wy will equal the bottom coordinate of the subwindow (in units from
    // the bottom of the overall window) if this is the Qt OSX emulator.
...
static void skin_window_show_opengles(SkinWindow* window, bool deleteExisting) {
...
static void skin_window_ensure_fully_visible(void* ptr) {
    ...
    if (!skin_winsys_is_window_fully_visible()) {
        ...
        if (VERBOSE_CHECK(init)) {
            ...
            win_w = getConsoleAgents()->surface->skin_surface_width(data->window->surface);
            win_h = getConsoleAgents()->surface->skin_surface_height(data->window->surface);

            dprint("Window was not fully visible: "
                   "monitor=[%d,%d,%d,%d] window=[%d,%d,%d,%d]",
                   data->monitor.pos.x, data->monitor.pos.y,
                   data->monitor.size.w, data->monitor.size.h, win_x, win_y,
                   win_w, win_h);
        }
...
void skin_window_enable_touch(SkinWindow* window, int enabled) {
    window->enable_touch = !!enabled;
}

void skin_window_enable_trackball(SkinWindow* window, int enabled) {
    window->enable_trackball = !!enabled;
}

void skin_window_enable_dpad(SkinWindow* window, int enabled) {
    window->enable_dpad = !!enabled;
}

void skin_window_enable_qwerty(SkinWindow* window, int enabled) {
    window->enable_qwerty = !!enabled;
}

void skin_window_set_title(SkinWindow* window, const char* title) {
    if (window && title) {
        skin_winsys_set_window_title(title);
    }
}

void skin_window_position_changed(SkinWindow* window, int x, int y) {
    window->x_pos = x;
    window->y_pos = y;
}

int skin_window_recompute_subwindow_rect(SkinWindow* window,
                                         SkinRect* subwindow) {
    // The full subwindow must be intersected with the container to compute the
    // actual subwindow
    SkinRect result;
    ...
    // If the emulator window is so small that the native subwindow wouldn't
    // even appear, force the subwindow to at least have positive size, else the
    // native window managers may crash. For example, on Linux, X11 crashes when
    // told to create a window with 0 size.
...
void skin_window_scroll_updated(SkinWindow* window,
                                int dx,
                                int xmax,
                                int dy,
                                int ymax) {
    ...
    int right_buf = getConsoleAgents()->surface->skin_surface_width(window->surface) -
                    (window->framebuffer.w + left_buf);
    float px;
    if (left_buf + right_buf >= xmax) {
        // The subwindow fits entirely in the container, so there are no
    ...
    int bottom_buf = getConsoleAgents()->surface->skin_surface_height(window->surface) -
    ...
    if (top_buf + bottom_buf >= ymax) {
        ...
        py = (float)((ymax - dy) - bottom_buf) /
             (float)(ymax - (bottom_buf + top_buf));
    }

    // Clamp the values to [0,1]. (dx,dy) = (0,0) indicates to align the bottom
    // left corner of the framebuffer with the bottom left corner of the
    // subwindow. (1,1) indicates to align the top right corner of the
    // framebuffer with the top right corner of the subwindow. All intermediate
    // values are an interpolation between these two states.
    px = px > 1 ? 1 : (px < 0 ? 0 : px);
    py = py > 1 ? 1 : (py < 0 ? 0 : py);
...
static void skin_window_resize(SkinWindow* window, int resize_container) {
    ...
    window->surface = getConsoleAgents()->surface->skin_surface_resize(window->surface, window_w, window_h,
                                          layout_w, layout_h);

    // Force redraw the background and skin to avoid drawing empty frames. This
    // reduces flicker on resize and rotate.
    ...
    getConsoleAgents()->surface->skin_surface_get_scaled_rect(window->surface, &drect, &drect);
...
static int skin_window_reset_internal(SkinWindow* window, SkinLayout* slayout) {
    Layout layout;
    ...
    if (disp != NULL) {
        if (slayout->onion_image) {
            ...
            adisplay_set_onion(disp, slayout->onion_image,
                               slayout->onion_rotation, slayout->onion_alpha);
        } else if (window->onion) {
            // Onion was specified via command line.
            adisplay_set_onion(disp, window->onion, window->onion_rotation,
                               window->onion_alpha);
        }
    }
...
int skin_window_reset(SkinWindow* window, SkinLayout* slayout) {
...
void skin_window_zoomed_window_resized(SkinWindow* window,
                                       int dx,
                                       int dy,
                                       int w,
                                       int h,
...
void skin_window_set_lcd_brightness(SkinWindow* window, int brightness) {
    ADisplay* disp = window->layout.displays;
...
void skin_window_free(SkinWindow* window) {
    if (window) {
        getConsoleAgents()->surface->skin_surface_unrefp(&window->surface);
...
void skin_window_set_onion(SkinWindow* window,
                           SkinImage* onion,
                           SkinRotation onion_rotation,
                           int onion_alpha) {
    ADisplay* disp;
...
void skin_window_set_layout_region(SkinWindow* window,
                                   int xOffset,
                                   int yOffset,
                                   int width,
                                   int height) {
    int displayIdx;
    for (displayIdx = 0; displayIdx < window->layout.num_displays;
         displayIdx++) {
...
void skin_window_set_display_region_and_update(SkinWindow* window,
                                               int xOffset,
                                               int yOffset,
                                               int width,
                                               int height) {
...
void skin_window_set_scale(SkinWindow* window, double scale) {
    ...
    window->scroll_h = 0;
    window->zoom =
            1.0;  // Scaling the window should reset all "viewport" parameters

    skin_window_resize(window, 1);
    skin_window_redraw(window, NULL);
}

void skin_window_set_zoom(SkinWindow* window,
                          double zoom,
                          int dw,
                          int dh,
                          int scroll_h) {
...
void skin_window_redraw(SkinWindow* window, SkinRect* rect) {
    if (window != NULL && window->surface != NULL) {
...

        {
            ...
            if (skin_rect_intersect(&r, rect, &layout->rect)) {
                getConsoleAgents()->surface->skin_surface_fill(window->surface, &r, layout->color);
            }
        }
        ...
        getConsoleAgents()->surface->skin_surface_update(window->surface, rect);
...
void skin_window_process_touch_event(SkinWindow* window, SkinEvent* ev) {
    ...
    if (ev->type == kEventTouchBegin || ev->type == kEventTouchEnd ||
        ...
        if (ev->type == kEventTouchBegin) {
            ...
            index = getIndexOfMultitouchPoint(ev->u.multi_touch_point.id,
                                              window);
        }
        if (index == -1) {
            return;
    ...
    switch (ev->type) {
        ...
        default:
            derror("%s: Invalid enum value passed to function. Only accepts: "
                   "kEventTouchBegin,kEventTouchEnd,kEventTouchUpdate",
                   __FUNCTION__);
            break;
    }
}
    ...
    switch (ev->type) {
        ...
        case kEventMouseMotion:
            ...
            if (window->drag_x_start == 0 && window->drag_y_start == 0) {
                ...
                if (finger->at_corner) {
                    // Resize: just update the overlay until the mouse button is
                    // released
                    skin_winsys_paint_overlay_for_resize(mx, my);
                    break;
                }
                // The user is dragging the window
                int posX = window->window_x_start + ev->u.mouse.x_global -
                           window->drag_x_start;
                int posY = window->window_y_start + ev->u.mouse.y_global -
                           window->drag_y_start;
                skin_winsys_set_window_pos(posX, posY);
                break;
            }
            if (window->ball.tracking) {
                skin_window_trackball_move(window, ev->u.mouse.xrel,
                                           ev->u.mouse.yrel);
                break;
            }
            mx = ev->u.mouse.x;
...
void skin_window_process_pen_event(SkinWindow* window, SkinEvent* ev) {
    Button* button;
    int mx, my;
    ...
    switch (ev->type) {
        case kEventPenPress:
            ...
            if (pen->inside) {
                // The click is inside the touch screen
                pen->tracking = 1;
                add_pen_event(window, pen, pen->pos.x, pen->pos.y, ev,
                              button_state);
            } else if (!multitouch_should_skip_sync(button_state)) {
                // This is a single click outside the touch screen
                window->button.pressed = NULL;
        ...
        case kEventPenMove:
            ...
            if (window->drag_x_start == 0 && window->drag_y_start == 0) {
                ...
                // The user is dragging the window
                int posX = window->window_x_start + ev->u.pen.x_global -
                           window->drag_x_start;
                int posY = window->window_y_start + ev->u.pen.y_global -
                           window->drag_y_start;
                skin_winsys_set_window_pos(posX, posY);
                break;
            }
...
static ADisplay* skin_window_display(SkinWindow* window) {
    return window->layout.displays;
}

void skin_window_update_display(SkinWindow* window,
                                int x,
                                int y,
                                int w,
                                int h) {
...
void skin_window_update_gpu_frame(SkinWindow* window,
    ...
    // Convert from GL_RGBA to 32-bit ARGB.
    {
        ...
        for (; dst < dst_end; src += 4, dst += 1) {
            dst[0] = ((uint32_t)src[3] << 24) | ((uint32_t)src[0] << 16) |
...
int skin_window_add_display(SkinWindow* window,
                            uint32_t id,
                            uint32_t w,
                            uint32_t h) {
    FingerState* fs = NULL;
