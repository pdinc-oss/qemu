...
#include "aemu/base/CpuUsage.h"                               // for CpuUsage
#include "aemu/base/Log.h"                                    // for base
#include "aemu/base/async/ThreadLooper.h"                     // for ThreadL...
#include "aemu/base/memory/MemoryTracker.h"                   // for MemoryT...
...
#include "host-common/constants.h"                            // for ANDROID...
#include "host-common/refcount-pipe.h"                        // for android...
#include "host-common/window_agent.h"                         // for gQAndro...
...
static bool setup_console_and_adb_ports(int console_port,
    ...
    if (legacy_adb) {
        agents->net->slirpRedir(false, adb_port, 5555);
    } else {
    ...
    if (getConsoleAgents()->settings->android_qemu_mode()) {
        ...
        switch (flavor) {
            ...
            case AVD_DESKTOP:
                ...
                break;
            case AVD_DEV_2024:
                crashhandler_add_string("AVD_FLAVOR", "AVD_DEV_2024");
                break;
            case AVD_OTHER:
